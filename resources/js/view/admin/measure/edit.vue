        ...
        <div class="content container-fluid">
            ...
            <div class="page-header">
                <div class="row align-items-center">
                    <div class="col">
                        <h3 class="page-title">{{$t('sidebar.Measurement')}}</h3>
                        <ul class="breadcrumb">
                            <li class="breadcrumb-item"><router-link :to="{name: 'indexMeasure'}">{{$t('global.UnitMeasurement')}}</router-link></li>
                            <li class="breadcrumb-item active">{{$t('permission.measureUnit edit')}}</li>
                        </ul>
                    </div>
                </div>
            </div>
    ...
    setup(props){
        ...
        let getMeasure = () => {
            ...
            adminApi.get(`/v1/dashboard/measure/${id.value}/edit`)
                .then((res) => {
                    let l = res.data.data;
                    addMeasure.data.name_ar = l.measure.name_ar;
                    addMeasure.data.name_en = l.measure.name_en;
                })
                .catch((err) => {
        ...
        return {loading,...toRefs(addMeasure),v$,t};
    },
    methods: {
        editMeasure(){
            this.v$.$validate();
