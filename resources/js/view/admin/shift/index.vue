<template>
    <div  :class="['page-wrapper',this.$i18n.locale == 'ar'? 'page-wrapper-ar':'']">
        <div class="content container-fluid">
                ...
                <div class="row align-items-center">
                    <div class="col">
                        ...
                        <ul class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active">{{ $t('global.workShifts') }}</li>
                        </ul>
                    </div>
                            ...
                            <div class="table-responsive">
                                <table class="table mb-0">
                                    ...
                                    <tr v-for="(item,index) in shifts" :key="item.id">
                                        <td>{{ index + 1 }}</td>
                                        <td>{{ item.name }}</td>
                                        <td>{{parseInt(item.type) ? $t('global.PM') : $t('global.AM')}}</td>
                                        <td>{{ item.start }}</td>
                                        <td>{{ item.end }}</td>
                                        <td>
                                            <a href="#" @click="activationShift(item.id,item.name,item.active,index)">
                                        ...
                                        <td>
                                            ...
                                            <router-link
                                                :to="{name: 'editShift', params: {id:item.id}}"
                                            ...
                                            <a
                                                href="#"
                                                @click="deleteShift(item.id, item.name, index)"
            ...
            <!-- /Table -->
            <!-- start Pagination -->
            <Pagination :limit="2" :data="shiftsPaginate" @pagination-change-page="getShift">
                <template #prev-nav>
...
export default {
    ...
    setup() {
        ...
        let getShift = (page = 1) => {
            ...
            adminApi.get(`/v1/dashboard/shift?page=${page}&search=${search.value}`)
                .then((res) => {
                    let l = res.data.data;
        ...
        function activationShift(id, jobName, active,index) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    adminApi.get(`/v1/dashboard/activationShift/${id}`)
                        .then((res) => {
                            Swal.fire({
                                icon: 'success',
                            ...
                            shifts.value[index]['active'] =  active ? 0:1
                        })
                        .catch((err) => {
                            Swal.fire({
                                icon: 'error',
        ...
        function deleteShift(id, roleName, index) {
            Swal.fire({
        ...
        return {shifts, loading, getShift, search, activationShift,permission, shiftsPaginate,deleteShift};
