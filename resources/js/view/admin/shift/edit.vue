        ...
        <div class="content container-fluid">
            ...
            <div class="page-header">
                <div class="row align-items-center">
                    <div class="col">
                        <h3 class="page-title">{{$t('global.workShifts')}}</h3>
                        <ul class="breadcrumb">
                            <li class="breadcrumb-item"><router-link :to="{name: 'indexShift'}">{{$t('global.workShifts')}}</router-link></li>
                        ...
                        <div class="card-body">
                            ...
                            <div class="row">
                                <div class="col-sm">
                                    ...
                                    <div class="alert alert-danger text-center" v-if="errors['name_en']">{{ errors['name_en'][0] }}<br /> </div>                                    <div class="alert alert-danger text-center" v-if="errors['type']">{{ errors['type'][0] }}<br /> </div>
                                    ...
                                    <form @submit.prevent="editShift" class="needs-validation">
                                        <div class="form-row row">
                                            ...
                                            <div class="col-md-6 mb-3">
                                                <label>{{ $t('global.ShiftType') }}</label>

                                                <select v-model="data.type" :class="['form-select',{'is-invalid':v$.type.$error,'is-valid':!v$.type.$invalid}]">
                                                    <option :value="0">{{$t('global.AM')}}</option>
                                                    <option :value="1">{{$t('global.PM')}}</option>
                                                </select>
                                                <div class="valid-feedback">{{$t('global.LooksGood')}}</div>
                                                <div class="invalid-feedback">
                                                    <span v-if="v$.type.required.$invalid">{{$t('global.ShiftTypeIsRequired')}}<br /> </span>
                                                </div>
                                            ...
                                            <div class="col-md-6 mb-3">
                                                ...
                                                <input type="time" class="form-control"
                                                       ...
                                                       :class="{'is-invalid':v$.start.$error,'is-valid':!v$.start.$invalid}"
                                                >
                                                <div class="valid-feedback">{{$t('global.LooksGood')}}</div>
                                                <div class="invalid-feedback">
                                                    <span v-if="v$.start.required.$invalid">{{$t('global.StartsFromIsRequired')}}<br /> </span>
                                                </div>
                                            </div>
                                            ...
                                            <div class="col-md-6 mb-3">
                                                ...
                                                <input type="time" class="form-control"
                                                       ...
                                                       :class="{'is-invalid':v$.end.$error,'is-valid':!v$.end.$invalid}"
                                                >
                                                <div class="valid-feedback">{{$t('global.LooksGood')}}</div>
                                                <div class="invalid-feedback">
                                                    <span v-if="v$.end.required.$invalid">{{$t('global.endsInIsRequired')}}<br /> </span>
                                                </div>
                                            </div>
    ...
    setup(props){
        ...
        let getMainShiftViews = () => {
            ...
            adminApi.get(`/v1/dashboard/shift/${id.value}/edit`)
                .then((res) => {
                    let l = res.data.data;
                    addShift.data.name_ar = l.shift.name_ar;
                    addShift.data.name_en = l.shift.name_en;
                    addShift.data.type = l.shift.type;
                    addShift.data.start = l.shift.start;
                    addShift.data.end = l.shift.end;
                })
                .catch((err) => {
        ...
        return {t,loading,...toRefs(addShift),v$};
    },
    methods: {
        editShift(){
            this.v$.$validate();
            ...
            if(!this.v$.$error){
                ...
                adminApi.put(`/v1/dashboard/shift/${this.id}`,this.data)
                    .then((res) => {
                        notify({
