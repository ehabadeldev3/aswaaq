        ...
        <div class="content container-fluid">
            ...
            <div class="page-header">
                <div class="row align-items-center">
                    <div class="col">
                        <h3 class="page-title">{{$t('global.dispatcher')}}</h3>
                        <ul class="breadcrumb">
                            <li class="breadcrumb-item"><router-link :to="{name: 'indexDispatcher'}">{{$t('global.dispatcher')}}</router-link></li>
                                ...
                                <div class="col-sm">
                                    ...
                                    <form @submit.prevent="editEmployee" class="needs-validation">
                                        <div class="form-row row">
                                            ...
                                            <div class="col-md-6 mb-3 "  v-if="suppliers.length">
                                                ...
                                                :searchable="true"
                                                :closeOnSelect="false"
                                                :class="{'is-invalid':v$.supplier_id.$error || errors['supplier_id'],'is-valid':!v$.supplier_id.$invalid && !errors['supplier_id']}"
                                                valueProp="id"
                                                />

                                                <div class="valid-feedback">{{$t('global.LooksGood')}}</div>
                                                <div class="invalid-feedback">
                                                    <span v-if="v$.supplier_id.required.$invalid"> {{ $t('global.IsRequired') }}<br /> </span>
                                                </div>
                                            </div>


                                            <div class="col-md-6 mb-3 "  v-if="all_permissions.length">
                                                <label class="typo__label">{{$t('global.Additional dispatcher permissions')}}</label>
                                                <Multiselect
...
export default {
    ...
    setup(props){
        ...
        const get_all_additional_dispatcher_permissions = async () => {
            await adminApi.get('/v1/dashboard/get_all_additional_dispatcher_permissions')
            .then((result) => {
        ...
        let getMainEmployeeViews = () => {
            ...
            adminApi.get(`/v1/dashboard/dispatcher/${id.value}/edit`)
                .then((res) => {
                    let l = res.data.data;
                    user_id.value = l.dispatcher.user_id;
                    addEmployee.data.name = l.dispatcher.user.name;
                    addEmployee.data.email = l.dispatcher.user.email;
                    addEmployee.data.phone = l.dispatcher.user.phone;
                    addEmployee.data.address = l.dispatcher.address;
                    addEmployee.data.National_ID = l.dispatcher.National_ID;
                    addEmployee.data.birth_date = l.dispatcher.birth_date;
                    addEmployee.data.hiring_date = l.dispatcher.hiring_date;
                    addEmployee.data.salary = l.dispatcher.salary;
                    addEmployee.data.permissions = l.permissions;
                    image.value = l.media;

                    l.dispatcher.suppliers.forEach(el =>{
                        addEmployee.data.supplier_id .push(el.id);
                    })
                })
                .catch((err) => {
        ...
        const get_suppliers = async () => {
            await adminApi.get('/v1/dashboard/activeSupplier')
            .then((result) => {
                result.data.data.suppliers.forEach(element => {
                    suppliers.value.push({id:element.id,text:element.name})
                })
            })
        ...
        onBeforeMount(() => {
            ...
            get_all_additional_dispatcher_permissions();
        });
        ...
        return {t,user_id,preview,imageUpload,image,numberOfImage,suppliers,loading,...toRefs(addEmployee),v$,all_permissions};
    },
    methods: {
        editEmployee(){
            this.v$.$validate();
            ...
            if(!this.v$.$error){
                ...
                adminApi.post(`/v1/dashboard/dispatcher/${this.id}`,formData)
                    .then((res) => {
